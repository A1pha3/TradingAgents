...
注意：TradingAgentsGraph 不承担具体分析逻辑。它只负责"装配"和"协调"，不负责"判断"。这是一种清晰的职责隔离。

## 状态模型：系统的共享数据契约

AgentState 是这个项目里最关键的数据契约。它继承自 LangGraph 的 `MessagesState`（自动提供 `messages` 字段），把原本可能散落在消息历史中的信息沉淀成结构化字段。

| 字段 | 类型 | 作用 |
| ---- | ---- | ---- |
| messages | MessagesState 内置 | 图执行中的消息历史（由 LangGraph 自动管理） |
| company_of_interest | str | 当前标的（如 `"NVDA"`、`"7203.T"`） |
| trade_date | str | 当前分析日期（如 `"2024-05-10"`） |
| sender | str | 当前消息发送者的角色标识 |
| market_report | str | Market Analyst 的技术分析结果 |
| sentiment_report | str | Social Media Analyst 的情绪分析结果 |
| news_report | str | News Analyst 的新闻分析结果 |
| fundamentals_report | str | Fundamentals Analyst 的基本面分析结果 |
| investment_debate_state | InvestDebateState | 投资辩论状态（含历史、计数、裁决） |
| investment_plan | str | Research Manager 裁决后的投资计划 |
| trader_investment_plan | str | Trader 生成的执行计划 |
| risk_debate_state | RiskDebateState | 风险辩论状态（含三方历史、计数、裁决） |
| final_trade_decision | str | Portfolio Manager 的最终决策文本 |

下游节点读取上下文时，不必从自由文本里猜测"上一阶段到底产出了什么"，而是直接读取字段。

### InvestDebateState：投资辩论状态

`InvestDebateState` 是 Bull/Bear Researcher 辩论过程中的结构化状态：

| 字段 | 类型 | 作用 |
| ---- | ---- | ---- |
| bull_history | str | Bull Researcher 的累积发言记录 |
| bear_history | str | Bear Researcher 的累积发言记录 |
| history | str | 双方合并的完整对话历史 |
| current_response | str | 最近一条发言的内容（用于判断下一个发言者是 Bull 还是 Bear） |
| judge_decision | str | Research Manager 的裁决结果 |
| count | int | 当前已完成的发言次数（达到 `2 × max_debate_rounds` 时转入 Research Manager） |

**初始值**（来自 `Propagator.create_initial_state`）：所有字符串字段为空，`count` 为 0。

### RiskDebateState：风险辩论状态

`RiskDebateState` 是三方风险辩论过程中的结构化状态：

| 字段 | 类型 | 作用 |
| ---- | ---- | ---- |
| aggressive_history | str | Aggressive Analyst 的累积发言记录 |
| conservative_history | str | Conservative Analyst 的累积发言记录 |
| neutral_history | str | Neutral Analyst 的累积发言记录 |
| history | str | 三方合并的完整对话历史 |
| latest_speaker | str | 最近发言者标识（用于轮转路由：Aggressive → Conservative → Neutral → ...） |
| current_aggressive_response | str | Aggressive Analyst 最近一次的完整发言 |
| current_conservative_response | str | Conservative Analyst 最近一次的完整发言 |
| current_neutral_response | str | Neutral Analyst 最近一次的完整发言 |
| judge_decision | str | Portfolio Manager 的最终决策 |
| count | int | 当前已完成的发言次数（达到 `3 × max_risk_discuss_rounds` 时转入 Portfolio Manager） |

**初始值**：所有字符串字段为空，`count` 为 0。

**`latest_speaker` 的路由作用**：`ConditionalLogic.should_continue_risk_analysis` 通过检查 `latest_speaker` 的前缀决定下一个发言者。以 `”Aggressive”` 开头则转 Conservative，以 `”Conservative”` 开头则转 Neutral，否则转 Aggressive。
...
图执行的输入不是一段裸文本，而是一份结构化运行上下文。

### get_graph_args：图执行参数配置

`Propagator.get_graph_args()` 返回传递给 LangGraph 执行引擎的参数字典：

```python
{
    "stream_mode": "values",             # 流式模式，逐步返回状态快照
    "config": {
        "recursion_limit": 100,          # 防止无限循环的最大递归深度
    },
}
```

`stream_mode: "values"` 意味着每次图节点执行完毕后，都会产出一份完整的当前状态快照。这使得 CLI 可以实时追踪每个阶段的输出进度。

### propagate 的两种执行路径

`TradingAgentsGraph.propagate()` 根据 `debug` 参数选择不同的执行路径：

| 模式 | 调用方式 | 行为 |
| ---- | ---- | ---- |
| `debug=True` | `self.graph.stream(init_state, **args)` | 逐步流式执行，每个 chunk 包含部分状态更新，CLI 可实时展示 |
| `debug=False` | `self.graph.invoke(init_state, **args)` | 一次性执行完毕，返回最终状态 |

两种模式的最终结果一致——都返回 `(final_state, decision)` 元组。区别仅在于中间过程是否可见。

**debug 模式的流式行为**：在 `debug=True` 时，`propagate()` 会遍历 `graph.stream()` 产出的每个 chunk，对最后一条消息调用 `pretty_print()`，并将所有 chunk 追加到 `trace` 列表中。最终的 `final_state` 取自 `trace[-1]`，即最后一个快照。
...
selected_analysts 的顺序会影响 Analyst 执行顺序 — 它不只是一个"启用集合"，同时也是"执行序列定义"。

正因如此，GraphSetup 是扩展指南中的关键文件。只要你打算新增 Analyst、调整阶段顺序、让某个节点变成可选能力，最后都绕不开 [tradingagents/graph/setup.py](../../tradingagents/graph/setup.py)。
...
系统中存在一个容易混淆的设计细节：**工具注册**和**工具绑定**是两个独立步骤。

第一层是 `TradingAgentsGraph._create_tool_nodes()` 中注册的 ToolNode。每个 ToolNode 包含一组可执行的工具函数。以 `news` ToolNode 为例，它注册了三个工具：
...
"news": ToolNode([
    get_news,
    get_global_news,
    get_insider_transactions,  # 在 ToolNode 中注册
])
```

第二层是 Analyst 工厂函数中通过 `llm.bind_tools()` 绑定到 LLM 的工具子集。以 `create_news_analyst` 为例：

```python
# tradingagents/agents/analysts/news_analyst.py
tools = [
    get_news,
    get_global_news,           # 只绑定了 2 个工具
    # 注意：get_insider_transactions 未被绑定
]
chain = prompt | llm.bind_tools(tools)
```

这就是双层设计的结果：`get_insider_transactions` 虽然在 ToolNode 中注册了，但 News Analyst 的 LLM 不会主动调用它 — 因为 LLM 不知道这个工具的存在。

各 Analyst 实际绑定的精确工具列表如下：

| Analyst | ToolNode 中注册 | LLM 实际绑定 |
| ---- | ---- | ---- |
| Market | `get_stock_data`, `get_indicators` | `get_stock_data`, `get_indicators` |
| Social Media | `get_news` | `get_news` |
| News | `get_news`, `get_global_news`, `get_insider_transactions` | `get_news`, `get_global_news` |
| Fundamentals | `get_fundamentals`, `get_balance_sheet`, `get_cashflow`, `get_income_statement` | `get_fundamentals`, `get_balance_sheet`, `get_cashflow`, `get_income_statement` |

注意：News Analyst 的 ToolNode 注册了 `get_insider_transactions`，但 LLM 并未绑定该工具。当前只有 Fundamentals Analyst 的 `fundamentals_analyst.py` 在其 prompt 中引用了 `get_insider_transactions`，但实际上它的 `tools` 列表中也没有包含此工具——它只绑定了 4 个财务工具。
...
6 种 Provider 仅需要 3 种客户端实现，显著降低了维护成本。
...
| `get_memories(current_situation, n_matches=2)` | 当前状况（`n_matches` 默认值为 1，但所有调用方显式传入 2） | top-n 匹配记录 | Bull/Bear Researcher, Research Manager, Trader, Portfolio Manager |
...
记忆在后续运行时被读取：角色通过 `memory.get_memories(curr_situation, n_matches=2)` 检索最相似的 2 个历史场景，将反思结论注入分析上下文。注意：`get_memories` 方法的默认 `n_matches` 参数为 1，但所有调用方（Researcher、Research Manager、Trader、Portfolio Manager）均显式传入 `n_matches=2`。
...
默认配置中存在 results_dir，但当前 `_log_state` 实际写入的是 eval_results。这是一个历史问题：结果输出路径在“配置声明”和“实际实现”之间还没有完全对齐。它不会阻止系统运行，但影响工程一致性和使用者预期。
...
1. 打开 `tradingagents/graph/trading_graph.py`，找到 `_create_tool_nodes` 方法，对比各 Analyst 的 ToolNode 注册列表。然后打开 `tradingagents/agents/analysts/news_analyst.py`，确认 `tools` 列表中的工具是否与 ToolNode 注册完全一致。如果不一致（例如 `get_insider_transactions` 在 ToolNode 中注册但 LLM 未绑定），这种设计有什么好处和潜在问题？
