...
3. 新增角色时，很容易把停止条件、上下文格式和执行顺序一起改乱。
...
更具体地说，Analyst 内部循环的停止条件不是“模型觉得说完了”，而是最后一条消息里是否还包含 tool_calls。只要还有工具调用请求，流程就会在 Analyst 节点和对应 ToolNode 之间往返，直到进入消息清理节点。
