...
这篇文档不讲安装细节，它回答的核心问题是：为什么这个项目采用多 Agent 而非一个超级 Prompt？为什么用图结构管理流程而不是让 Agent 自由对话？为什么 Analyst、Researcher、Trader、Risk Manager 要拆成不同阶段？工具调用、辩论轮数、最终裁决分别扮演什么角色？

如果你想真正理解 TradingAgents 的设计逻辑，这一篇是关键。

读完本文后，你应该能够：解释多 Agent 设计的必要性；理解 LangGraph 在其中的工作流控制作用；说清各阶段之间的输入输出关系；分析辩论轮数、工具节点和审批节点如何共同塑造系统行为。

## 原理一：把复杂金融决策拆成角色协作

TradingAgents 把金融研究和交易决策拆成多个具有独立职责的角色。关键不在于模型数量，而在于每个角色有清晰的职责边界：
...
这个拆分解决了多 Agent 系统中的一个根本问题：单个 Agent 同时承担信息收集、观点生成、反方挑战和最终裁决时，提示词负担过重，推理边界模糊。

## 原理二：图编排比自由对话更可控

项目使用 LangGraph 组织执行流程，而不是让多个 Agent 漫无边界地轮流说话。图编排的关键收益：
...
系统由图定义什么条件下进入下一阶段，不依赖模型自行判断何时停止。对研究框架来说，这种可控性非常重要。
...
四类 Analyst 默认都不是直接从世界知识中空想结论。它们会先通过 ToolNode 调用数据工具，再基于工具结果生成报告。这样设计使得：
...
不是所有节点都值得支付最高推理成本。高频节点更关心吞吐和响应速度，关键裁决节点更关心推理质量和稳定性。
...
它的作用是给后续复盘和长期演进预留一个稳定接口，而不是立即让系统“自动变聪明”。

BM25 的局限：它更擅长词面匹配，不擅长强语义泛化。当前阶段优先用 BM25 而非向量检索，原因如下：
...
研究经理负责将争论收敛为投资计划——其目标不是统计发言数量，而是从对立观点中提取可执行的结论。
...
研究结论正确不代表行动风险可接受——这是风险辩论阶段独立存在的根本理由。
...
没有 ToolNode，系统只是"多角色聊天"；有 ToolNode，才是"多角色研究工作流"。这是两个本质不同的系统。
...
循环次数完全由模型行为决定 — 模型认为不需要更多数据时，自动结束工具调用阶段。
...
max_debate_rounds 和 max_risk_discuss_rounds 不只是性能开关，它们直接影响系统行为风格。理解它们的关键在于**乘数机制** — 每个参数控制的不是总发言次数，而是每位参与者的发言次数。
