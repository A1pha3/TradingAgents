...
默认配置中存在 results_dir，但实际状态日志当前写入 eval_results。这是一个历史问题：结果输出路径在"配置声明"和"实际实现"之间还没有完全对齐。它不会阻止项目使用，但会给使用者和维护者带来理解偏差。
...
__文档元信息__
难度：⭐⭐⭐ | 类型：工程实践 | 更新日期：2026-04-07 | 预计阅读时间：45 分钟
