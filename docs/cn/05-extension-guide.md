...
- 按完整流程新增一个分析师（Analyst），覆盖节点 → 状态 → 图结构 → CLI 的全链路
- 新增数据供应商和 LLM Provider，并理解供应商差异如何收敛在边界层
- 调整工作流结构（如并行执行、新增中间节点）而不破坏图编译
- 替换记忆系统实现，同时保持上层接口兼容
...
跳过这个清单，扩展过程通常会退化成"哪里报错修哪里"。

## 扩展一：新增一个分析师（Analyst）

以新增宏观分析师（Macro Analyst）为例，建议按下面的顺序操作。

### 第 1 步：新增节点实现

在 tradingagents/agents/analysts 下新建 macro_analyst.py。以下是一个完整可运行的模板，遵循现有分析师的标准写法：
...
时间紧时保底走完 MVP：图能编译 → 最小执行路径跑通 → 新字段写回 state → CLI 或日志看到新产物。缺任意一步，都只算“代码写完了”，不算扩展完成。
...
TradingAgents 的扩展能力取决于你是否尊重它现有的边界设计。沿着状态、节点、边界层和 CLI 暴露四条主线做闭环修改，比"哪能跑通改哪"更可靠。

## 自测问题

1. 如果你在新增分析师时忘了在 `propagation.py` 的 `create_initial_state` 中添加对应字段的空字符串初始化，会发生什么？
2. `output_language` 的 `get_language_instruction()` 在 English 时返回空字符串——为什么要这样设计？
3. 新增 Provider 时，如果你的客户端没有在 `factory.py` 中注册，系统会在什么时候报错？初始化阶段还是运行阶段？
4. 为什么记忆系统的接口兼容性（`add_situations` 和 `get_memories` 的签名）比内部实现更重要？

## 练习题

1. 按照本文的宏观分析师模板，创建一个 `sentiment_analyst.py`（与现有 `social_media_analyst` 类似但专注于情绪量化），列出你需要修改的所有文件清单。
2. 编写一个测试用例，验证当你同时传入 `google_thinking_level` 和 `openai_reasoning_effort` 时，只有与当前 `llm_provider` 匹配的参数会生效。

---
__文档元信息__
难度：⭐⭐⭐⭐ | 类型：专家设计 | 更新日期：2026-05-23 | 预计阅读时间：55 分钟
