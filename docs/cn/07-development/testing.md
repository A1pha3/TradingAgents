...
TradingAgents 的测试套件有 51 个测试文件，但它们要回答的核心问题只有一个：**在没有真实 API key 和网络的情况下，怎么验证这套接了十几个外部服务的多 Agent 系统仍然是对的？**
...
整套测试套件以**纯单元测试**为主，全部用 mock LLM 和 mock 网络实现。`pytest -m unit` 能跑通绝大多数用例（39 个文件标注了 `unit` marker），不依赖任何真实 API key 或网络。这一选择背后的考虑是：
...
| 主题 | 代表文件 | 覆盖内容 |
|------|---------|---------|
| 记忆系统 | `test_memory_log.py`（最大，871 行） | `TradingMemoryLog` 全部路径：存储、延迟反思、PM 注入、轮换 |
...
| 报告 | `test_reporting.py` | CLI 与编程式 API 报告一致（#1037） |
| LLM Provider | `test_provider_registry.py`、`test_openai_compatible_provider.py`、`test_openrouter_model_select.py`、`test_bedrock_provider.py`、`test_deepseek_reasoning.py`、`test_anthropic_effort.py`、`test_google_api_key.py`、`test_google_thinking_level.py`、`test_minimax.py`、`test_ollama_base_url.py`、`test_openai_responses_base_url.py`、`test_openai_reasoning_effort.py`、`test_capabilities.py`、`test_model_validation.py` | 20 个 provider 的注册、能力表、base_url 解析、key 处理 |
| 数据流 | `test_vendor_routing.py`、`test_vendor_errors.py`、`test_alpha_vantage_hardening.py`、`test_fred.py`、`test_polymarket.py`、`test_reddit_fallback.py`、`test_stocktwits_resilience.py`、`test_yfinance_stale_ohlcv_guard.py`、`test_market_data_validator.py`、`test_no_data_handling.py`、`test_safe_ticker_component.py` | vendor 路由、错误体系、各家 vendor 的边界情况 |
| 图与 Agent | `test_structured_agents.py`、`test_analyst_execution.py`、`test_risk_router_path_map.py`、`test_checkpoint_resume.py`、`test_signal_processing.py`、`test_market_toolnode.py`、`test_news_analyst_prompt.py`、`test_instrument_identity.py` | 图的形状、Agent 结构化输出、风险辩论路由、断点续跑 |
