...
**问题动机**：用户把输出语言设成中文后，辩论环节要不要也切中文？还是只翻译最终报告、辩论保持英文？

**原理剖析**：TradingAgents 选择**全链路本地化**——设成中文后，分析师、研究员、辩手、裁判、交易员的文本输出全部用中文，包括两轮辩论。技术实现上，`get_language_instruction()`（`agent_utils.py:52-65`）在 `output_language` 非 English 时返回 `" Write your entire response in {lang}."`，拼接到每个产出报告内容的 Agent 的 prompt 末尾。

**源码解析**：这个函数的 docstring 写明它作用于 "analysts, researchers, debaters, research manager, trader, and portfolio manager"——即所有 12 个 LLM 角色。English 时返回空字符串（不加 token），非 English 时注入语言指令。所以非英文运行产出的是一份完全本地化的报告，而不是英文辩论加中文摘要的拼凑。

**权衡**：主流 LLM 的训练语料以英文为主，非英文下推理质量可能略低——金融术语映射、逻辑链条、结构化输出填充都有退化风险，且这些退化会累积到最终决策。框架选择优先保证报告的一致性和可读性：让用户拿到一份完整的中文（或日文、韩文）报告，而不是在英文辩论和中文摘要之间来回切换。如果你对推理质量的敏感度高于可读性，可以保持 `output_language=English`。多语言配置细节见 [多语言输出](../02-user-guide/output-language.md)。
