...
一个框架的架构文档能告诉你"它长什么样"，但常常漏掉更有用的那一半："它为什么长成这样，以及它在哪些地方**故意不**这么做"。本文收集了 TradingAgents 里 11 个有明确取舍意味的设计决策，每个都按"问题动机 → 原理剖析 → 源码解析"的顺序展开。
...
**源码解析**：核心是 `yfinance_news.py` 里的 `_in_news_window`（`yfinance_news.py:66-72`）：
...
第二个规则是 #992 的修复重点。早期 yfinance 返回的"flat 结构"文章没有发布时间，导致它们绕过日期过滤，把未来新闻泄露进历史窗口。现在的实现会给 flat 文章也解析出 `pub_date`（`yfinance_news.py:45-60`），让它们同样可过滤。这套逻辑有专门的回归测试（`tests/test_news_lookahead.py`），覆盖"未来文章被排除""无日期文章在回测里被排除""实盘窗口保留无日期文章"等场景。
...
这 11 个决策串起来，构成了 TradingAgents 的设计判断。要继续深入：
