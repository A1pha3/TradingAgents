...
读完这篇，你应该能回答：一次 `propagate()` 调用在系统里经历了什么、为什么有 12 个 LLM 角色（外加 1 个非 LLM 的 Msg Clear 节点，共 13 个图节点）却只调 2 个 LLM、以及三层横向机制各自负责什么。
...
| 13 | Msg Clear（每个选中的分析师各 1 次，最多 4 次） | 横切 | – | – | – | 清空消息历史，留下锚定到标的+日期的占位消息 |
