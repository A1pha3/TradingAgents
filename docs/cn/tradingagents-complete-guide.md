...
目前仓库中的显式测试非常有限，主要覆盖 ticker 符号标准化和交易所后缀保留。这说明项目当前更像研究原型，而不是经过严格测试矩阵验证的企业级平台。
...
1. 从根目录的 main.py 入手，先跑通最小示例。
2. 再结合 cli/main.py 观察交互式执行体验。
3. 然后阅读 tradingagents/graph 下的 5 个核心模块，理解流程编排。
4. 最后进入 agents、dataflows、llm_clients，评估你准备扩展的具体切入点。
