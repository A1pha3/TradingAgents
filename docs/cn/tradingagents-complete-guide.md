...
这里的"研究深度"不是抽象标签，而是直接映射到辩论轮数 — CLI 中的研究深度调高，本质上就是增加多 Agent 讨论的回合数。
...
每类 Analyst 默认只能访问与其角色匹配的工具集合 — 这是一种"最小必要能力"原则，有助于减少误用工具与提示词偏移。
...
Propagator.create_initial_state 会创建一个包含默认消息、标的、日期、辩论状态和四类报告空位的字典。Graph 的输入不是自由文本，而是一份结构化上下文。
