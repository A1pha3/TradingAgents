...
**关键细节：最长优先。** 元组顺序是 `("USDT", "USDC", "USD")`，匹配时从长到短。如果先匹配 `USD`，那么 `BTCUSDT` 会被错误地切分成 `BTCUS` + `T` 之外的逻辑——准确说，`BTCUSDT` 会先匹配到末尾的 `USD`（位于 `...USDT` 中），但因为我们想要的是剥离整个 `USDT`，所以必须先试 `USDT` 再试 `USD`。这是一个容易写错的顺序敏感点。
...
**错误理解：** 「`US30`（道琼斯）的前后 3 位都是字母，会被外汇规则误判。」
**正确理解：** 外汇规则的判定不是「6 个字母」，而是「6 个字母且前后各 3 位**都在 `_FOREX_CURRENCIES` 里**」。`US3` 和 `0` 都不是货币代码，所以 `US30` 不会走外汇分支——它会在更早的别名表分支命中 `^DJI`。外汇规则是最后兜底的，别名表的精确匹配优先级更高。
