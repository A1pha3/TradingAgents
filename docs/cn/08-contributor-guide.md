...
对这个项目最有价值的贡献，不一定是"再加一个新 Agent" — 补一层验证、统一一处边界、修复一个长期的工程不一致点，都会比加角色更关键。
...
如果你想快速建立风险感，可以把它们对应成 6 类真实踩坑案例：
...
这 5 步和四条主线的关系可以简单理解为：第 1 步定位主线，第 2 步列出受影响文件面，第 3 步先闭合状态与图主线，第 4 步验证边界与暴露主线，第 5 步让文档和代码重新对齐。
...
最直接的检查方法是：跑一条最小路径，然后直接检查 final_state 和落盘 JSON，而不是只看终端摘要。
...
最直接的检查方法是：至少验证“启用该能力”和“禁用该能力”两条路径，而不是只测一种配置。
...
这里不要只看 happy path。至少补一条异常路径，例如限流、未知模型或 provider 参数缺失。
...
最直接的检查方法是：同一能力分别从 CLI 和 Python API 走一遍最小路径，看配置表达和结果产物是否一致。
...
如果你需要把这 5 步进一步操作化，可以这样执行：

1. 先确认文档、字段名和示例代码没有和当前实现脱节。
2. 再跑最小 API 路径，验证主链路。
3. 再跑与你改动直接相关的最小路径，例如一个工具调用、一个 provider 初始化或一个 CLI 选择。
...
先让结构性改动落地并稳定，再引入行为性变化。因为两类变化一旦混在一起，你很难判断问题来自结构重组还是行为逻辑变化。

如果你不确定一个改动属于哪一类，可以用这个简单标准判断：

1. 只改变文件组织、状态字段、接口形状、节点连接关系，但不打算改变输出语义，算结构性改动。
2. 改变模型选择、条件逻辑、辩论轮数、工具返回解释方式，算行为性改动。
